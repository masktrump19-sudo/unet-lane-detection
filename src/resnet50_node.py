...
class LaneDetectionRKNN:
    def __init__(self, model_path, num_points=4):
        ...
        # 创建ROS订阅者和发布者
        self.image_sub = rospy.Subscriber('/camera/warped_image', Image, self.image_callback, queue_size=1)
        self.result_pub = rospy.Publisher('/lane_detection/points', Float32MultiArray, queue_size=1)
        self.result_image_pub = rospy.Publisher('/lane_detection/result_image', Image, queue_size=1)
        
        print("ROS publishers and subscribers initialized")
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # 执行车道检测推理
            points, inference_time = self.inference(cv_image)
            
            # 发布检测结果点
            self.publish_points(points, msg.header)
            
            # 生成并发布结果图像
            result_image = self.visualize_result(cv_image, points)
            self.publish_result_image(result_image, msg.header)
            
            # 打印性能信息
            if inference_time > 0:
                fps = 1.0 / inference_time
                rospy.loginfo(f"Lane detection FPS: {fps:.2f}, Inference time: {inference_time*1000:.2f}ms")
    ...
    def publish_points(self, points, header):
        ...
        # 将点坐标扁平化为一维数组 [x1, y1, x2, y2, x3, y3, x4, y4]
        for point in points:
            msg.data.extend([float(point[0]), float(point[1])])
        
        self.result_pub.publish(msg)
    
    def publish_result_image(self, result_image, header):
        """
        发布可视化结果图像
        """
        try:
            ...
            rospy.logerr(f"Error publishing result image: {e}")
...
def main():
    ...
    try:
        rospy.loginfo("Lane detection node is running. Waiting for image messages...")
        rospy.loginfo("Subscribed to: /camera/warped_image")
        rospy.loginfo("Publishing to: /lane_detection/points and /lane_detection/result_image")
